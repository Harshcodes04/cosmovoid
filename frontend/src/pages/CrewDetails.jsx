...
const getNationalityLabel = (nationality) => {
  ...
  return (
    nationality.nationality_name || nationality.name || nationality.abbrev || ""
  );
};

const formatDuration = (isoStr, showDaysOnly = false) => {
  if (!isoStr || !isoStr.startsWith("P")) return isoStr;
  const match = isoStr.match(
    /P(?:(\d+)Y)?(?:(\d+)M)?(?:(\d+)D)?(?:T(?:(\d+)H)?(?:(\d+)M)?(?:(\d+)S)?)?/,
  );
  if (!match) return isoStr;

  const [_, y, mo, d, h, m, s] = match;

  let totalDays = parseInt(d || 0, 10);
  if (y) totalDays += parseInt(y, 10) * 365;
  if (mo) totalDays += parseInt(mo, 10) * 30;

  if (showDaysOnly) return totalDays.toString();
...
const getImageUrl = (image) => {
  ...
  return image.image_url || image.thumbnail_url || image.url || image.src || "";
};
          ...
          <div className="space-y-10">
            ...
            <div className="flex flex-col items-center gap-8 sm:flex-row sm:items-start">
              ...
              <div className="flex-1 text-center sm:text-left">
                <div className="flex flex-wrap items-center justify-center gap-2 mb-3 sm:justify-start">
                  <span
                    className={`rounded-full border px-3 py-1 text-[10px] font-medium uppercase tracking-[0.18em] ${statusClass}`}
                  >
                    {statusName}
                  </span>
                  ...
                  {astronaut.time_in_space && (
                    <div className="text-center">
                      <p className="text-xl font-semibold text-white font-mono">
                        {formatDuration(astronaut.time_in_space, true)}
                      </p>
                      <p className="text-[10px] uppercase tracking-[0.2em] text-zinc-600">
                        Days in Space
                      </p>
            ...
            <Section title="Personal Details">
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                <Stat
                  label="Date of Birth"
                  value={fmtDate(astronaut.date_of_birth)}
                />
                <Stat label="Nationality" value={astronaut.nationality} />
                <Stat
                  label="Date of Death"
                  value={fmtDate(astronaut.date_of_death)}
                />
                <Stat
                  label="First Flight"
                  value={fmtDate(astronaut.first_flight)}
                />
                <Stat
                  label="Last Flight"
                  value={fmtDate(astronaut.last_flight)}
                />
                <Stat
                  label="Total EVA Time (Spacewalk)"
                  value={formatDuration(astronaut.eva_time)}
                  mono
                />
              </div>
            </Section>
              ...
              <Section title={`Spacewalks (${astronaut.spacewalks.length})`}>
                <div className="space-y-2">
                  {astronaut.spacewalks.map((sw, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between rounded-2xl border border-white/8 bg-white/3 px-5 py-3.5"
                    >
