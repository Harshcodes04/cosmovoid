...
const FEATS = [
  {
    n: "01",
    tag: "LAUNCH INTELLIGENCE",
    h: "Proof humans are unbelievably dramatic engineers.",
    p: "Live countdowns, mission data, launch failures, and enough acronyms to confuse normal people.",
    to: "/launches",
    flip: false,
  },
  {
...
const EXPLORE_SECTIONS = [
  ...
  },
  {
    ...
    label: "Rockets",
    to: "/rockets",
    color: "from-violet-500/15 to-purple-700/5",
  ...
  },
  {
    ...
    label: "Crew",
    to: "/crew",
    color: "from-emerald-500/15 to-teal-700/5",
  ...
  },
  {
    ...
    label: "Gallery",
    to: "/gallery",
    color: "from-pink-500/15 to-rose-700/5",
  ...
  },
  {
    ...
    label: "Asteroids",
    to: "/asteroids",
    color: "from-orange-500/15 to-amber-700/5",
  ...
  },
  {
    ...
    label: "Events",
    to: "/events",
    color: "from-sky-500/15 to-indigo-700/5",
  ...
  },
  {
    ...
    label: "News",
    to: "/news",
    color: "from-yellow-500/15 to-lime-700/5",
  ...
  },
  {
    ...
    label: "About",
    to: "/about",
    color: "from-zinc-500/15 to-slate-700/5",
  ...
  },
  {
    ...
    label: "Contact",
    to: "/contact",
    color: "from-fuchsia-500/15 to-purple-700/5",
  ...
  return (
    <div className="scanline relative overflow-hidden rounded-3xl border border-white/10 bg-[#080808] shadow-[0_40px_80px_rgba(0,0,0,0.7)]">
      ...
      <div className="flex items-center gap-2.5 border-b border-white/8 px-5 py-3.5">
        ...
        <span className="ml-3 text-[10px] uppercase tracking-[0.3em] text-zinc-600">
          Mission Control
        </span>
        <span className="ml-auto flex items-center gap-1.5 text-[9px] text-cyan-400 uppercase tracking-widest">
          <span className="gp h-1.5 w-1.5 rounded-full bg-cyan-400" />
          Live
        </span>
      </div>
      ...
      <div className="border-b border-white/6 px-5 py-4">
        <p className="font-mono text-[9px] uppercase tracking-[0.28em] text-zinc-600 mb-3">
          Upcoming Missions
        </p>
        {loading ? (
          ...
          next2.map((l) => (
            <div
              key={l.id}
              className="mb-2.5 rounded-xl border border-white/6 bg-white/3 px-3.5 py-2.5"
            >
              <div className="flex items-center justify-between gap-2 w-full">
                <p className="text-xs font-semibold text-white leading-snug flex-1 min-w-0 truncate">
                  {l.name}
                </p>
              ...
              <p className="mt-0.5 text-[10px] text-zinc-600 truncate w-full">
                {l.launch_service_provider?.abbrev ||
                  l.launch_service_provider?.name ||
                  "—"}
                {l.rocket?.configuration?.name
                  ? ` · ${trim(l.rocket.configuration.name, 20)}`
                  : ""}
              </p>
            </div>
          ))
      ...
      <div className="px-5 py-4">
        ...
        {loading ? (
          ...
          neos.map((a) => {
            const miss = Number(
              a.close_approach_data?.[0]?.miss_distance?.astronomical || 0,
            ).toFixed(3);
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2">
                  <span
                    className={`h-1.5 w-1.5 rounded-full ${hazard ? "bg-red-400" : "bg-zinc-500"}`}
                  />
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [apodR, astR, globalR, newsR, jR] =
        await Promise.allSettled(imports);
      if (!live) return;
      const newsArr = newsR.status === "fulfilled" ? newsR.value.data : null;
      setLiveData({
        globalLaunchCount:
          globalR.status === "fulfilled" ? globalR.value.data?.count || 0 : 0,
        globalUpcoming:
          globalR.status === "fulfilled"
            ? globalR.value.data?.results || []
            : [],
        news: Array.isArray(newsArr)
          ? newsArr[0]
          : newsArr?.results?.[0] || null,
        apod: apodR.status === "fulfilled" ? apodR.value.data : null,
        asteroids: astR.status === "fulfilled" ? neos(astR.value.data) : [],
        journalEntries:
          jR?.status === "fulfilled" ? jR.value.data?.entries || [] : [],
        loading: false,
      });
    })();
    return () => {
          ...
          <div className="relative z-10 mx-auto grid max-w-7xl w-full gap-8 px-4 sm:px-6 md:px-10 lg:grid-cols-2 lg:items-stretch lg:gap-16 lg:px-16 xl:px-20 min-h-[600px]">
            ...
            <div className="min-w-0 self-start -mt-4 lg:-mt-8">
              ...
              <div className="ha ha6 mt-6 flex flex-wrap gap-3">
                ...
                <Link
                  to={sec.to}
                  className="inline-flex items-center rounded-full border border-white/10 bg-white/5 px-7 py-3.5 text-sm font-medium text-zinc-300 backdrop-blur-sm transition-colors hover:bg-white/10 hover:text-white"
                >
                  {sec.label}
                </Link>
              </div>
        ...
        <div className="overflow-hidden border-y border-white/5 bg-white/2 py-3">
          <div className="mqr">
            {[...MQ_WORDS, ...MQ_WORDS].map((w, i) => (
              <span
                key={i}
                className={`text-[10px] uppercase tracking-[0.28em] ${w === "·" ? "text-zinc-700" : "text-zinc-600"}`}
              >
                {w}
              </span>
            ))}
              ...
              <div className={`group grid gap-12 lg:grid-cols-2 lg:items-center`}>
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <span className="font-mono text-5xl font-bold text-white/6 leading-none">
                          {f.n}
                        </span>
                        <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[9px] uppercase tracking-[0.28em] text-zinc-500">
                          {f.tag}
                        </span>
                      </div>
                      <h3 className="text-3xl font-light tracking-[-0.04em] text-white lg:text-4xl">
                        {f.h}
                      </h3>
                        ...
                        <div className="hov-lift rounded-2xl border border-white/8 bg-zinc-950 p-6 shadow-[0_20px_50px_rgba(0,0,0,.5)]">
                          ...
                          {liveData.loading ? (
                            [0, 1, 2].map((k) => (
                              <div
                                key={k}
                                className="mb-3 h-14 animate-pulse rounded-xl bg-white/4"
                              />
                            ))
                          ) : liveData.globalUpcoming.length ? (
                            liveData.globalUpcoming.slice(0, 3).map((l) => {
                              ...
                              return (
                                <div
                                  key={l.id}
                                  className="mb-3 w-full overflow-hidden rounded-xl border border-white/6 bg-white/3 px-4 py-3"
                                >
                            ...
                            <>
                              {liveData.apod.media_type === "image" ? (
                                <div className="overflow-hidden rounded-2xl border border-white/8">
                                  <img
                                    src={liveData.apod.url}
                                    alt={liveData.apod.title}
                                ...
                                <div className="overflow-hidden rounded-2xl border border-white/8 bg-zinc-900 p-4">
                                  ...
                                  <p className="text-[10px] text-zinc-600 mt-1">
                                    Today's APOD · {liveData.apod.media_type}
                                  </p>
                                </div>
                              )}
                              <div className="grid grid-cols-2 gap-2">
                                {[
                                  ...
                                  <Link
                                    key={label}
                                    to={to}
                                    className="rounded-xl border border-white/6 bg-zinc-950 p-3 transition-colors hover:border-white/12 hover:bg-white/5"
                                  >
                                    <p className="text-xs font-medium text-white">
                                      {label}
                                    </p>
                        ...
                        <div className="hov-lift space-y-3">
                          {liveData.loading ? (
                            ...
                            liveData.journalEntries.slice(0, 3).map((e) => (
                              <div
                                key={e._id}
                                className="rounded-2xl border border-white/8 bg-zinc-950 p-4 shadow-[0_8px_24px_rgba(0,0,0,.4)]"
                              >
                                <div className="flex items-center justify-between gap-2 mb-2">
                                  ...
                                  {e.mood && (
                                    <span className="shrink-0 rounded-full border border-white/10 px-2 py-0.5 text-[9px] text-zinc-500">
                                      {e.mood}
                                    </span>
                                  )}
                                </div>
                                <p className="text-xs leading-5 text-zinc-600 line-clamp-2">
                                  {e.content}
                                </p>
                              </div>
                            ))
                            ...
                            <>
                              {[
                                [
                                  "Witnessed Starship IFT-6 today",
                                  "reflective",
                                  "Incredible footage of the booster catch...",
                                ],
                                [
                                  "Pillars of Creation — APOD",
                                  "curious",
                                  "The Webb image dropped today. I've been staring at it for an hour...",
                                ],
                                [
                                  "First journal entry",
                                  "excited",
                                  "Starting this log to keep track of every launch I follow...",
                                ],
                              ].map(([title, mood, body]) => (
                                <div
                                  key={title}
        ...
        <section className="relative flex flex-col justify-center min-h-[100svh] px-4 py-16 sm:px-6 sm:py-20 md:px-10 md:py-24 lg:px-16 xl:px-20 overflow-hidden snap-start">
          ...
          <div className="relative mx-auto max-w-7xl w-full">
            <div className="rv mb-12 sm:mb-16 flex flex-col sm:flex-row items-start sm:items-end justify-between gap-6">
              ...
              <Link
                to="/explore"
                className="shrink-0 rounded-full border border-white/10 bg-white/4 px-5 py-2.5 text-sm text-zinc-400 transition-colors hover:bg-white/8 hover:text-white"
              >
                Full directory →
              </Link>
            </div>
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-6">
              {EXPLORE_SECTIONS.map((s) => (
                ...
                >
                  <span
                    className={`flex size-9 items-center justify-center rounded-xl border bg-black/20 ${s.border} ${s.text}`}
                  >
                    <s.Icon size={14} />
                  </span>
                  <div>
                    ...
                    <p
                      className={`text-[10px] mt-0.5 transition-colors group-hover:opacity-100 opacity-60 ${s.text}`}
                    >
                      Open →
                    </p>
                  </div>
          ...
          <div className="mx-auto max-w-7xl w-full grid grid-cols-2 gap-6 sm:gap-8 lg:grid-cols-4 relative z-10">
            {[
              [
                liveData.loading
                  ? "…"
                  : liveData.globalLaunchCount > 0
                    ? `${liveData.globalLaunchCount}`
                    : "Live",
                "Upcoming global missions",
              ],
              [
              ...
              [
                liveData.loading
                  ? "…"
                  : user
                    ? liveData.journalEntries.length > 0
                      ? `${liveData.journalEntries.length}`
                      : "0"
                    : "Private",
        ...
        <section className="relative flex flex-col justify-center min-h-[100svh] px-4 py-16 sm:px-6 sm:py-20 md:px-10 md:py-24 lg:px-16 xl:px-20 overflow-hidden snap-start">
          ...
          <div className="relative mx-auto max-w-3xl w-full text-center">
            ...
            <div className="rv d3 mt-10 flex flex-wrap items-center justify-center gap-4">
              <Link
                to={cta.to}
                className="hov-lift group inline-flex items-center gap-2 rounded-full bg-white px-9 py-4 text-sm font-semibold text-zinc-950"
              >
                {cta.label}
                <span className="transition-transform duration-300 group-hover:translate-x-0.5">
                  →
              ...
              <Link
                to="/explore"
                className="inline-flex items-center rounded-full border border-white/10 bg-white/4 px-9 py-4 text-sm font-medium text-zinc-400 backdrop-blur-sm transition-all hover:bg-white/8 hover:text-white"
              >
                Explore first
              </Link>
            </div>
            ...
            <p className="rv d5 mt-16 text-[10px] text-zinc-200 uppercase tracking-widest">
              Built during late nights and existential crises.
            </p>
          </div>
        </section>
      </main>
