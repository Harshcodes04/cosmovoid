...
const skyEventsService = require("../services/skyEvents.service");
...
exports.searchNasaMedia = asyncHandler(async (req, res, next) => {
  ...
  const data = await nasaService.searchNasaMedia(query, mediaType, page);
  res.json(data);
});
...
exports.getSkyEvents = (req, res) => {
  const data = skyEventsService.getSkyEvents();
  res.json(data);
};
