...
const getThreatLevel = (asteroid) => {
  ...
  return {
    ...
    bg: "bg-emerald-500/8 border-emerald-500/18",
    dot: "bg-emerald-400",
  };
};
    ...
    >
      <div className="flex flex-wrap items-center gap-4 px-5 py-4 sm:flex-nowrap">
        ...
        <div className="flex shrink-0 flex-col items-center gap-1.5">
          <span
            className={`h-2.5 w-2.5 rounded-full ${threat.dot} shadow-[0_0_8px_currentColor]`}
          />
          <span
            className={`text-[9px] font-semibold uppercase tracking-widest ${threat.color}`}
          >
            {threat.label}
          </span>
        </div>
        ...
        <div className="flex shrink-0 flex-wrap gap-x-6 gap-y-1 text-right">
          ...
          <div>
            <p className="font-mono text-xs text-zinc-400">
              {velocity
                ? `${Number(velocity).toLocaleString("en-IN", { maximumFractionDigits: 0 })} km/h`
                : "—"}
            </p>
            <p className="text-[10px] uppercase tracking-widest text-zinc-600">
              Velocity
      ...
      <div className="relative w-full max-w-2xl overflow-hidden rounded-[2rem] border border-white/10 bg-zinc-950 shadow-[0_30px_80px_rgba(0,0,0,0.6)]">
        ...
        <div className={`border-b border-white/8 px-6 py-5 ${threat.bg}`}>
          <div className="flex items-center gap-3 pr-12">
            ...
            <span
              className={`text-xs font-semibold uppercase tracking-widest ${threat.color}`}
            >
        ...
        <div className="mb-8 grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
          <div className="space-y-4">
            ...
            <p className="max-w-2xl text-sm leading-7 text-zinc-400">
              Real-time tracking data from NASA's Center for Near Earth Object
              Studies (CNEOS) for{" "}
              <strong className="text-zinc-200">{today}</strong>. Sorted by
          ...
          {!loading && !error && (
            <div className="grid grid-cols-3 gap-3 text-center lg:grid-cols-1 lg:text-right">
              ...
              <div>
                <p
                  className={`font-mono text-3xl font-light ${hazardousCount > 0 ? "text-red-400" : "text-emerald-400"}`}
                >
                  {hazardousCount}
                </p>
                <p className="text-[10px] uppercase tracking-widest text-zinc-600">
              ...
              <div>
                <p className="font-mono text-3xl font-light text-zinc-400">
                  {neos.length - hazardousCount}
                </p>
                <p className="text-[10px] uppercase tracking-widest text-zinc-600">
                  Classified safe
                </p>
